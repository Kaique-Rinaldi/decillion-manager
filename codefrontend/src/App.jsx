    ...
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginBottom:20}}>
        {PIPELINE_STAGE_KEYS.map(stageKey=>{
          ...
          return <div key={stageKey} onDragOver={e=>{e.preventDefault();setOverStage(stageKey)}} onDragLeave={()=>setOverStage(null)} onDrop={()=>onDrop(stageKey)} style={{background:isOver?stage.color+"14":"#111520",border:`1px solid ${isOver?stage.color+"60":"rgba(255,255,255,.06)"}`,borderRadius:10,padding:10,minHeight:120,transition:"all .15s"}}>
            ...
            <AnimatePresence>{stageDeals.map(d=><motion.div key={d.id} layout initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:.95}} draggable onDragStart={()=>setDraggingId(d.id)} style={{background:"#161b2a",border:"1px solid rgba(255,255,255,.06)",borderRadius:8,padding:8,marginBottom:6,cursor:"grab",opacity:draggingId===d.id?.45:1}}><div style={{fontSize:11,fontWeight:500,color:"#e8eaf0",marginBottom:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.name}</div><div style={{fontSize:10,color:"#5a6478",marginBottom:6}}>{d.company}</div><div style={{fontSize:11,fontWeight:600,color:"#22c97d",fontFamily:"monospace"}}>{formatCurrency(d.value)}</div><div style={{fontSize:8,color:"#5a6478",fontFamily:"monospace",marginTop:4,textAlign:"right"}}>{formatDate(d.createdAt)}</div></motion.div>)}</AnimatePresence>
          </div>
        })}
      </div>

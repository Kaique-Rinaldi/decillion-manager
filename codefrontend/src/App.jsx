  ...
  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        <StatCard label="Clientes em andamento" value={activeClients} delta={`de ${clients.length} total`}
          iconPath="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z" iconColor="blue"/>
        <StatCard label="Receita recebida" value={formatCurrency(totalRevenue)} delta="pagamentos confirmados"
          iconPath="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2zM12 6v6l4 2" iconColor="green"/>
        <StatCard label="Negociações em aberto" value={openDeals} delta={`${wonDeals.length} fechadas`}
          iconPath="M4 6h16M4 12h16M4 18h16" iconColor="amber"/>
        <StatCard label="Taxa de conversão" value={`${convRate}%`} delta="negociações ganhas / total"
          ...
          <div style={{ display:"flex", alignItems:"flex-end", gap:5, height:80 }}>
            {REVENUE_DATA.map((r,i) => (
              <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center" }}>
                <div style={{ width:"100%", borderRadius:"4px 4px 0 0", height:`${(r.value/maxR)*100}%`,
              ...
              return (
                <div key={c.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 0",
                  ...
                  <div style={{ width:28, height:28, borderRadius:7, background:pal.bg, color:pal.fg,
                    display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, flexShrink:0 }}>{initials(c.name)}</div>
                  <div style={{ flex:1, minWidth:0 }}>
              ...
              {[...deals].sort((a,b) => b.value - a.value).slice(0,5).map(d => {
                ...
                return (
                  <tr key={d.id} style={{ borderBottom:"1px solid rgba(255,255,255,.03)" }}>
                    ...
                    <td style={{ padding:"7px 0" }}><span style={{ fontSize:9, fontFamily:"monospace", color:st.color }}>{st.label}</span></td>
                    <td style={{ padding:"7px 0", fontSize:12, color:"#22c97d", fontFamily:"monospace", textAlign:"right" }}>{formatCurrency(d.value)}</td>
                  </tr>
                )
              })}
            </tbody>
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:16 }}>
        <button onClick={openCreate} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 16px", borderRadius:8,
          background:"#4f6ef7", border:"none", color:"#fff", fontSize:12, fontWeight:500, cursor:"pointer", fontFamily:"inherit" }}>
          + Nova negociação
        </button>
      </div>
        ...
        {PIPELINE_STAGE_KEYS.map(stageKey => {
          ...
          return (
            <div key={stageKey} onDragOver={e => onDragOver(e, stageKey)} onDragLeave={() => setOverStage(null)} onDrop={() => onDrop(stageKey)}
              ...
              {stageDeals.length > 0 && <div style={{ fontSize:9, color:"#5a6478", fontFamily:"monospace", marginBottom:8 }}>{formatCurrency(stageValue)}</div>}
              <AnimatePresence>
                {stageDeals.map(d => (
            ...
            <motion.div initial={{ scale:.93, opacity:0, y:10 }} animate={{ scale:1, opacity:1, y:0 }} exit={{ scale:.93, opacity:0 }} transition={{ duration:.18 }} onClick={e => e.stopPropagation()}
              ...
              <form onSubmit={handleSave} noValidate style={{ padding:"20px 24px 24px" }}>
                <div style={{ display:"grid", gap:12 }}>
                  {[{k:"name",l:"Nome da negociação *",t:"text",ph:"Ex: Redesign site Acme"},{k:"company",l:"Empresa",t:"text",ph:"Ex: Acme Corp"}].map(({k,l,t,ph}) => (
                    <div key={k}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    <div>
                      ...
                      <input type="number" min="0" step="0.01" value={form.value||""} onChange={e => setF("value", e.target.value)} placeholder="0,00"
                    ...
                    <div>
                      <div style={{ fontSize:9, color:"#5a6478", fontFamily:"monospace", textTransform:"uppercase", letterSpacing:".5px", marginBottom:5 }}>Etapa *</div>
                      <select value={form.stage||""} onChange={e => setF("stage", e.target.value)}
                        ...
                        {PIPELINE_STAGE_KEYS.map(k => <option key={k} value={k}>{PIPELINE_STAGE[k].label}</option>)}
                      </select>
  ...
  return (
    <Card title="Central de notificações" style={{ padding:0 }}>
      <div style={{ padding:"12px 16px", borderBottom:"1px solid rgba(255,255,255,.05)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <button style={{ fontSize:10, background:"none", border:"1px solid rgba(255,255,255,.08)", color:"#8892a4", padding:"4px 10px", borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>
      ...
      {notifs.map((n,i) => (
        <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"12px 16px",
          ...
          <div style={{ position:"relative", flexShrink:0 }}>
            <div style={{ width:32, height:32, borderRadius:8, background:n.color+"18", color:n.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13 }}>{n.icon}</div>
            {n.unread && <div style={{ position:"absolute", top:-2, right:-2, width:8, height:8, borderRadius:"50%", background:"#4f6ef7", border:"2px solid #111520" }}/>}
          </div>
          <div>
  ...
  function Section({ icon, title, children }) {
    return (
      <div style={{ background:"#111520", border:"1px solid rgba(255,255,255,.06)", borderRadius:12, overflow:"hidden", marginBottom:12 }}>
        <div style={{ padding:"12px 16px", borderBottom:"1px solid rgba(255,255,255,.05)", fontSize:11, fontWeight:600, color:"#e8eaf0", display:"flex", alignItems:"center", gap:8 }}>
  ...
  return (
    <div>
      <Section icon="👤" title="Minha conta">
        <Row label="Email" desc="Conta autenticada via Supabase" right={<span style={{ fontSize:11, color:"#5a6478", fontFamily:"monospace" }}>{user?.email}</span>}/>
        <Row label="ID do usuário" desc="Identificador único" right={<span style={{ fontSize:9, color:"#5a6478", fontFamily:"monospace" }}>{user?.id?.slice(0,16)}…</span>}/>
...
export default function App() {
  ...
  if (!user) {
    return (
      <>
        <LoginPage onLogin={login} onRegister={register} loading={authLoading} error={authError} clearError={() => setError("")}/>
        ...
        <div style={{ padding:"14px 24px", borderBottom:"1px solid rgba(255,255,255,.06)", display:"flex", alignItems:"center", gap:12, background:"#111520", flexShrink:0 }}>
          ...
          {dataLoading && (
            <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:10, color:"#5a6478", fontFamily:"monospace" }}>
              <div style={{ width:10, height:10, borderRadius:"50%", border:"1.5px solid rgba(79,110,247,.3)", borderTopColor:"#4f6ef7", animation:"spin .6s linear infinite" }}/>
              Carregando…
            </div>
          )}
